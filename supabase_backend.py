...
def update_station(station_id: int, payload: dict):
    return get_supabase().table("stations").update(_records(payload)[0]).eq("id", station_id).execute()


def all_daily():
    return pd.DataFrame(
        get_supabase().table("daily_summary").select("*").order("trade_date", desc=True).execute().data or []
    )
...
def daily_rows(station_id: int | None = None, limit: int = 300):
    q = get_supabase().table("daily_summary").select("*").order("trade_date", desc=True).limit(limit)
    if station_id is not None:
        q = q.eq("station_id", station_id)
    return pd.DataFrame(q.execute().data or [])


def hourly_rows(station_id: int, trade_date: str):
    data = (
        get_supabase()
        .table("hourly_detail")
        .select("*")
...
def save_daily(station_id: int, filename: str, file_hash: str, summary: dict, hourly: pd.DataFrame, overwrite=False):
    ...
    daily = {
        ...
        "file_hash": file_hash,
        "lt_energy": float(summary["lt_energy"]),
        "lt_price": float(summary["lt_price"]),
        "actual_energy": actual,
        "spot_price": float(summary["spot_price"]),
        "unified_price": float(summary["unified_price"]),
        "spot_fee": float(summary["spot_fee"]),
        "lt_diff_fee": float(summary["lt_diff_fee"]),
        "energy_total": final,
        "final_revenue": final,
        "final_price": final / actual if actual else 0.0,
    }
        ...
        detail.append(
            {
                ...
                "trade_date": trade_date,
                "hour_no": int(r["时点"]),
                "lt_energy": float(r["中长期电量"]),
                "lt_price": float(r["中长期价"]),
                "actual_energy": float(r["上网电量"]),
                "spot_price": float(r["实时价"]),
                "unified_price": float(r["统一结算点价"]),
                "spot_fee": float(r["现货电费"]),
                "lt_diff_fee": float(r["中长期差价电费"]),
                "energy_total": float(r["电能量合计"]),
            }
        )
